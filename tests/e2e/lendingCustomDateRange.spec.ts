...
test.describe('Lending Custom Date Range & Pagination Interaction', () => {
	test('opens Custom Range UI, verifies inputs, validates invalid range, updates URL and resets page', async ({ page }) => {
		...
		// 6. Click Apply inside DateFilterMenu / Filter panel
		await applyBtn.click();

		// Also click FilterFooter Apply if open
	...
	test('resets page to 1 when applying a date range filter on page > 1', async ({ page }) => {
		...
		// Navigate to page 2 via URL if supported
		await page.goto('/lending?page=2');
	...
	test('verifies server-side pagination works for lending records', async ({ page }) => {
		await signInAndNavigateToLending(page);
