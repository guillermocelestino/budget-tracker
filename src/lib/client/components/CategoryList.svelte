<script lang="ts">
  ...
  import {
    ...
    getCategoryTint,
    getCategoryText,
  } from "$lib/shared/utils/categoryColors";
  ...
  async function saveEdit(cat: EnrichedCategory) {
    ...
    const clean =
      dots && dots.length > 1 ? raw.slice(0, raw.lastIndexOf(".")) : raw;
    ...
    try {
      ...
      formData.append(
        "budget_limit",
        budgetLimit != null ? String(budgetLimit) : "",
      );

      const resp = await fetch("?/budgetUpdate", {
        method: "POST",
        body: formData,
      });
      if (resp.ok) {
...
{#snippet compactRow(cat: EnrichedCategory, tint: string, fg: string)}
  <div class="compact-row">
    <span class="compact-icon" style="background: {tint}; color: {fg}"
      >{cat.icon}</span
    >
    <span class="compact-name">{cat.name}</span>
    <span class="compact-type" class:income={cat.type === "income"}
      >{cat.type === "income" ? "Income" : "Expense"}</span
    >
    <span class="compact-meta"
      >{cat.txnCount || 0} txns · {cat.recurringCount || 0} rec{cat.lastUsed
        ? ` · ${formatDateShort(cat.lastUsed)}`
        : ""}</span
    >
    <span class="compact-amount"
      >{formatCurrency(cat.type === "income" ? cat.earned : cat.spent)}</span
    >
...
{#if totalCount === 0}
  <div class="empty-state">
    ...
    <span
      >Categories help organize your income, expenses, budgets, and recurring
      schedules.</span
    >
    {#if onAdd}
      <button class="empty-cta" onclick={() => onAdd?.("expense")} type="button"
...
{:else}
  <div class="board" class:single={compact}>
    ...
    <section
      class="board-column board-column--expense flip7-card"
      class:inactive={activeType !== "expense"}
    ...
    >
      <header class="column-header">
        ...
        <span class="column-total">
          ...
          of {formatCurrency(
            expenseCategories.reduce((s, c) => s + c.budgeted, 0),
          )}
        </span>
      </header>
        ...
        <div class="category-list">
          {#each expenseCategories as cat (cat.id)}
            ...
            {:else}
              <div
                class="category-card"
                class:over-budget={statusClass(cat) === "over"}
              >
                ...
                <div class="card-header-area">
                  <div class="card-left">
                    <span
                      class="cat-icon"
                      style="background: {tint}; color: {fg}">{cat.icon}</span
                    >
                    <div class="cat-info">
                      <span class="cat-name">{cat.name}</span>
                      <span class="cat-type-badge coral">Expense</span>
                      <span class="cat-usage"
                        >{cat.txnCount || 0} transactions · {cat.recurringCount ||
                          0} recurring</span
                      >
                    </div>
                  </div>
                ...
                {#if cat.budget_limit != null}
                  <div class="budget-cluster">
                    <span class="cluster-item">
                      ...
                      {:else}
                        <button
                          class="cluster-value clickable"
                          onclick={() => startEdit(cat)}
                          title="Edit budget"
                          >{formatCurrency(cat.budgeted)}</button
                        >
                      {/if}
                    </span>
                    ...
                    <span class="cluster-item">
                      ...
                      <span class="cluster-value" class:inverted={cat.spent > 0}
                    ...
                    <span class="cluster-item">
                      ...
                      <span
                        class="cluster-value"
                        class:positive={cat.budgeted - cat.spent >= 0}
                        class:negative={cat.budgeted - cat.spent < 0}
                        >{formatCurrency(cat.budgeted - cat.spent)}</span
                      >
                    </span>
                ...
                {:else}
                  <div class="no-budget-cluster">
                    ...
                    <span class="cluster-sep" aria-hidden="true">·</span>
                    <button class="no-budget-set" onclick={() => startEdit(cat)}
                      >+ Set budget</button
                    >
                  </div>
                {/if}
                ...
                {#if cat.budget_limit != null && cat.budget_limit > 0}
                  <div class="progress-section">
                    <CategoryUsageBar
                      percent={budgetPercent(cat)}
                      status={statusClass(cat)}
                    />
                    <div class="progress-footer">
                      <span
                        class="pct-value"
                        class:ok={statusClass(cat) === "ok"}
                        class:warn={statusClass(cat) === "warn"}
                        class:over={statusClass(cat) === "over"}
                      >
      ...
      {:else}
        <div class="column-empty">
          ...
          {:else}
            ...
            <span
              >Track where money goes by adding your first expense category.</span
            >
          {/if}
        </div>
      {/if}
      {#if onAdd}
        <button
          class="column-add"
          type="button"
          onclick={() => onAdd?.("expense")}>+ Add Expense</button
        >
      {/if}
    </section>
    ...
    <!-- ══ INCOME column ══ -->
    <section
      class="board-column board-column--income flip7-card"
      class:inactive={activeType !== "income"}
    ...
    >
      <header class="column-header">
        ...
        <span class="column-total"
          >{formatCurrency(incomeCategories.reduce((s, c) => s + c.earned, 0))} earned</span
        >
      </header>
                ...
                <div class="card-body">
                  ...
                  <div class="card-right">
                    <span class="income-value"
                      >{formatCurrency(cat.earned)}</span
                    >
      ...
      {:else}
        <div class="column-empty">
          ...
          {:else}
            ...
            <span
              >See what's coming in by adding your first income category.</span
            >
          {/if}
        </div>
      {/if}
      {#if onAdd}
        <button
          class="column-add"
          type="button"
          onclick={() => onAdd?.("income")}>+ Add Income</button
        >
      {/if}
    </section>
  </div>
